...
import type { UniprotData } from "./model"
...
export const convertUniprotWebToModel = (data: any): UniprotData => {
  const scientificName = data.organism.names.find(
    (name: any) => name.type === "scientific",
  )
  const commonName = data.organism.names.find(
    (name: any) => name.type === "common",
  )
  let fullName = null
  let shortName = null
...
const get = (id: any): Promise<UniprotData> => {
  return http.get(`/proteins/${id}`).then((response) => {
